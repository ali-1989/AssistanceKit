...
class PathAssistance {
  ...
  static bool isStartBy(String master, String startBy) {
    if (TextAssistance.isEmptyOrNull(master) || TextAssistance.isEmptyOrNull(startBy)) {
      return false;
    }
  ...
  static String mergeUriIfNotSame(String? url1, String? url2) {
    if (TextAssistance.isEmptyOrNull(url1)) {
      if (TextAssistance.isEmptyOrNull(url2)) {
        return '';
      }

      return url2!;
    }
    else if (TextAssistance.isEmptyOrNull(url2)){
      return url1!;
    }

    String bigger = url1!.length > url2!.length? url1 : url2;
    String other = bigger == url1 ? url2 : url1;

    if(bigger.startsWith('/')){
