...
class GregorianDate extends ADateStructure {
  ...
  List<int> convertToGregorian() {
    return [getYear(), _backwardStepInRing(getMonth(), 1, 12, true), getDay()];
  }
  ...
  static GregorianDate convertDaysToDate(int days) {
    ...
    if (days > 0) {
      ...
      if (leftDays > 365) {
        ...
        var cDays = (year * 365) + (ADateStructure.getGregorianLeapYears(0, year+1).length);

        if((leftDays - cDays) > 366){
          year++;
          cDays = (year * 365) + (ADateStructure.getGregorianLeapYears(0, year+1).length);
        }
      ...
      if (leftDays > 365) {
        year = (leftDays-1) ~/ 365.2425;
        var cDays = (year * 365) + (ADateStructure.getGregorianLeapYears(-year, 0).length);

        if((leftDays - cDays) > 366){
          year++;
          cDays = (year * 365) + (ADateStructure.getGregorianLeapYears(-year, 0).length);
        }
