...
class SolarHijriDate extends ADateStructure {
  ...
  static bool isSolarLeapYearOld(int year) {
    if (year == 0) {
      return false;
    }
    ...
    var t1 = (y * 365.2421986).toInt();
    var t2 = ((y - 1) * 365.2421988).toInt();

    return (t1 - t2) == 366;
  }

  static bool isSolarLeapYear(int year) { //add by DeepSeek: 1404/01/18
    if (year == 0) {
      return false;
    }
    ...
    var t1 = y * 683;
    var t2 = (t1 % 2820).toInt();

    return t2 < 683;  // 1383,1387,1391,1395,1399,1403,1408 is leap
  }

  static bool _isSolarLeapYear(int year) {// add by DeepSeek & Qwen, not ok for big priod
    if (year == 0) {
      return false;
    }
    ...
    var t = (y % 33).toInt();
    const List<int> listLeap = [1, 5, 9, 13, 17, 22, 26, 30];

    return listLeap.contains(t);
  }

  /// [start, end)
  static List<int> getSolarLeapYears(int start, int end) {
  ...
  static SolarHijriDate convertDaysToDate(int days) {
    ...
    if (days > 0) {
      ...
      if (leftDays > 365) {
        ...
        year++;
	*/
          year = (leftDays-1) ~/ 365.2425;
          var cDays = (year * 365) + (getSolarLeapYears(0, year+1).length);

          if((leftDays - cDays) > 366){
            year++;
            cDays = (year * 365) + (getSolarLeapYears(0, year+1).length);
          }
      ...
      if (leftDays > 365) {
        year = (leftDays-1) ~/ 365.2425;
        var cDays = (year * 365) + (getSolarLeapYears(-year, 0).length);

        if((leftDays - cDays) > 366){
          year++;
          cDays = (year * 365) + (getSolarLeapYears(-year, 0).length);
        }
