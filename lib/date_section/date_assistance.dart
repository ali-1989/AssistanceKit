...
// Project imports:
import 'package:assistance_kit/date_formatter/src/date_format.dart';
import 'package:assistance_kit/date_section/time_zone.dart' as tz;
...
class DateAssistance {
	DateAssistance._();

	static DateTime Function()? overrideNow;

	static DateTime parse(String date){
		return DateTime.parse(date);
	}

  	/// >> 2025-11-16T14:13:08.448
	...
	static DateTime now(){
		return overrideNow?.call() ?? DateTime.now();
	}

	static DateTime nowWithUtcOffset(){
		final nowDt = now();
		var offset = nowDt.timeZoneOffset.inMilliseconds;

	    if(offset > 0){
	      offset *= -1;
	    }

		return DateTime.fromMillisecondsSinceEpoch(nowDt.millisecondsSinceEpoch + offset);
	}

	static DateTime addSubOffset(DateTime dt, int offsetMill){
    if(offsetMill > 0){
      offsetMill *= -1;
    }

		return DateTime.fromMillisecondsSinceEpoch(dt.millisecondsSinceEpoch + offsetMill);
	}

	static DateTime addSubOffsetToUtc(int offsetMill){
		final nowDt = now();
		var mOffset = nowDt.timeZoneOffset.inMilliseconds;
    ...
    mOffset += offsetMill;
		...
		return DateTime.fromMillisecondsSinceEpoch(nowDt.millisecondsSinceEpoch + mOffset);
	}

	static DateTime nowAsUtcTz(){
		return now().toUtc(); // same DateTime.timestamp()
	}

	static String nowToTimestamp(){
		final nowDt = now();
		return toTimestamp(nowDt);
	}

	static String nowWithUtcOffsetToTimestamp(){
		final now = nowWithUtcOffset();
	...
	/// 2025-11-16 14:13:08
	static String toTimestampWithoutMill(DateTime src){
    	   return DF.formatDate(src, [DF.yyyy, '-', DF.mm, '-', DF.dd, ' ', DF.HH, ':', DF.nn, ':', DF.ss]);
	}
	...
	static String fileNameByDate(){
		final now = nowAsUtcTz();
		return DF.formatDate(now, [DF.yyyy, '_', DF.mm, '_', DF.dd]);
	}

	static String fileNameByDateTime(){
		final now = nowAsUtcTz();
		return DF.formatDate(now, [DF.yyyy, '_', DF.mm, '_', DF.dd, '-', DF.hh,'_', DF.nn,'_', DF.ss, DF.uuu]);
	}

	static String fileNameByTime(){
		final now = nowAsUtcTz();
		return DF.formatDate(now, [DF.hh,'_', DF.nn,'_', DF.ss, DF.uuu]);
	}
	...
	static int daysSince(DateTime from) {
		return daysDifference(from, now().toUtc());
	}
	...
	static String formatYmdHm2(DateTime dt, {String divider = '-'}){
		return '${dt.year}/${dt.month}/${dt.day} $divider ${dt.hour}:${dt.minute.toString().padLeft(2, '0')}';
	}
	...
	static bool isToday(DateTime date, {bool utc = false}){
		return isSameYmd(date,utc? nowWithUtcOffset(): now());
	}
	...
	static bool isBeforeToday(DateTime d, {bool utc = true}){
    DateTime today;
    
    if(utc){
      today = DateAssistance.nowWithUtcOffset();
    }
    else {
      today = DateAssistance.now(); 
    }
	...
	static bool isBeforeEqualToday(DateTime d, {bool utc = true}){
    DateTime today;
	...
	static bool isAfterToday(DateTime d, {bool utc = true}){
    DateTime today;
	...
	static int getTodayDifferentDay(DateTime d, {bool utc = true}){
    DateTime today;
	...
	static bool isPastOf(DateTime? date, Duration dur){
		...
		return now().compareTo(future) > 0;
	}

	static bool isPastOfTime(DateTime? date1, DateTime? date2, Duration dur){
		if(date1 == null || date2 == null ){
			return true;
		}

		final future = date1.add(dur);

		return date2.compareTo(future) > 0;
	}
	...
	static String durationToTime(Duration dur){
		return dur.toString().split('.').first.padLeft(8, '0');
	}

	static String durationToHourMinuteReminder(Duration dur){
		return durationToTime(dur).substring(0, 5);
	}

	static String durationToMinuteSecond(Duration dur){
		return durationToTime(dur).substring(3, 8);
	}

	/// Duration(hours: 30, minutes: 10) >  30, 10
	static ({int hours, int remainderMinutes}) getHoursDistance(Duration d) {
		int hours = d.inHours;
