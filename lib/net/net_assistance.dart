...
// Project imports:
import 'package:assistance_kit/api/assistance/shell_assistance.dart';
...
class NetAssistance {
    ...
    if(System.isWindows()) {
      ...
      for (final line in lines) {
        if (line.indexOf(RegExp(r'Default Gateway')) > 0) {
          ...
          if (start > -1) {
            return TextAssistance.removeNonViewableFull(line.substring(start));
          }
        }
      }
    }
  ...
  static Future<String> getActiveIp() async{
    if(System.isWindows()) {
      ...
      for (final line in lines) {
        ...
        if (line.contains(RegExp(r'Default Gateway'))) {
          ...
          if (start > -1) {
            return TextAssistance.removeNonViewableFull(ipv4Line.substring(start));
          }
        }
      }
    ...
    // ip addr show/ ip -4 addr show dev eth0/ ip route/ hostname -I/ ifconfig eth0
    final shellRes = await ShellAssistance.shell('hostname', ['-I'], runInShell: true);
    ...
    for (final line in lines) {
      //r'link src'
      if (line.contains(RegExp(r'link',multiLine: false, caseSensitive: false))) {
        ipv4Line = line;
      }
    ...
    final regex = RegExp(r'(\d{1,3}\.){3}\d{1,3}');
    var match = regex.firstMatch(lines.first);
